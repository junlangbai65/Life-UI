...
> 写卡流程入口：本 skill（`tavern-cards`）。补充深参清单见 `.cursor/skills/mvu-zod-card-authoring/reference.md` 与 `workflow.md`（该 skill 的 `SKILL.md` 仅为短桩）。硬性 API 以 `.cursor/rules/mvu变量框架.mdc`、`mvu角色卡.mdc` 为准。
