...
本 skill 已安装于 `.cursor/skills/tavern-cards/`。在本仓库（tavern_helper_template）中：

- 在项目根目录运行 forge：`pnpm card:forge -- <command>`（详见 [project-integration.md](project-integration.md)）
- 角色卡项目目录：`cards/{Project}/`，注册文件：`.cardrc.json`
- 子代理位于 `.cursor/agents/`（check-agent、conversion-agent、first-message-agent）
- 前端界面/脚本在本仓库 `src/` 开发，与 forge 打包流程互补
...
**与 tavern-card-builder：** 造新卡/改造的方法论与分册以 `tavern-card-builder` 为准；本 skill 负责本仓库 `cards/` 工程落地与 forge。深参查阅用 `st-dev-guide-db`。
