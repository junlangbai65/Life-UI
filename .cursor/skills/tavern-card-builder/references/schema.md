...
教训：**初始化永远走 MVU 标准协议**——公共基线由 `[InitVar]`（enabled=false）扫描加载；固定多开局可由对应 greeting 的 `<initvar>` 提供完整分支初值，或用 `<UpdateVariable>` 在公共基线上做少量差异。绝不手写 `replaceMvuData` 一类“帮 MVU 初始化”的直写补丁。修复事故卡时只移除绕过初始化事件的脚本和已确认无意保留/格式错误的开场白变量块；合法逐 Swipe 初始化必须保留，并逐分支通过 schema 与 `swipes_data.stat_data` 对账。
