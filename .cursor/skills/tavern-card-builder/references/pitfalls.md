...
16. **手写"帮 MVU 初始化"补丁**：`replaceMvuData` 直写 0 楼绕过 `loadInitVarData()` → 初始化事件不发 → schema 默认值从未注入 → 五层连锁静默劣化。初始化只走 MVU 标准协议：公共基线用 `[InitVar]`，固定多开局用 greeting 内合法 `<initvar>` / `<UpdateVariable>`；禁止脚本直写伪造初始化。（schema.md §8）
