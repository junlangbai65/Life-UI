...
## 工作区协作边界（本仓库 tavern_helper_template）

本 skill 是 **「制卡规划与部件设计」** 入口（方法论 10 步 + references 分册），不替代工程流。

| 职责 | 用谁 |
|------|------|
| MVU/纯文字卡设计、访谈、schema/规则/世界书/状态栏方案 | **本 skill（tavern-card-builder）** |
| `cards/` 目录、forge pack、创作规划注册 | **`tavern-cards`** + `pnpm card:forge` |
| 源文件优先创意立项、长期经验沉淀 | **`agent-foundry-st`** + `agent-foundry-vault/50-创意库/` |
| Vue/webpack UI、`src/` 状态栏实现 | `.cursor/rules`（前端界面 / mvu*）+ `pnpm watch` |
| 精确 API / 避坑深参 | **`st-dev-guide-db`**（STDB）+ `@types`；冲突以当前 `@types` 与 Rules 为准 |
| 轻量 API 查阅 | `tavern-card-knowledge` |

落地工程产物时：优先写到本仓库 `cards/{Project}/` 与（可选）`src/{Project}/`，不要假设作者机器上的外部角色卡工作区路径。
...
5. **写开局协议与卡本体**——先区分两条合法路线：固定 `first_mes` / `alternate_greetings` 用 MVU 原生逐 Swipe 初始化（公共初值走 `[InitVar]`；完整分支初值走 `<initvar>`；少量差异可走 `<UpdateVariable>`），动态/向导式开局则把叙事协议（`[mvu_plot]`）与建档模板（`[mvu_update]`）分开，由真实用户开局消息触发。卡片字段与具体选择矩阵见 `references/card-writing.md`。
...
- 酒馆助手模板项目（含 `@types` 接口定义、示例角色卡、webpack 构建）：**本仓库即 tavern_helper_template**；`@types/iframe/exported.mvu.d.ts` 是 MVU 接口的权威签名。深参查阅用 skill `st-dev-guide-db`。
