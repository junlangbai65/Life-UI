...
- 如果文件夹中既有 `index.ts` 文件也有 `index.html` 文件, 则是前端界面项目. 例如, `示例/前端界面示例` 是一个前端界面项目.
- 如果文件夹中仅有 `index.ts` 文件, 则是脚本项目. 例如, `示例/脚本示例`、`示例/流式楼层界面示例` 是一个脚本项目.

你可以在 `初始模板/*/新建为src文件夹中的文件夹` 中找到前端界面和脚本项目的初始模板.

**webpack 仅扫描 `{示例,src}/**/index.{ts,tsx}` 作为打包入口.** 其它目录下的源码不会进入 `dist/`.

### 备份工作区（暂停/归档的项目）

`备份工作区/` 存放暂未启用的完整前端界面或脚本项目（如 ADven、Goth、LYsta 等）. 它们不参与打包; 恢复开发时将对应文件夹移回 `src/` 即可. 详见 `备份工作区/README.md`.

### Agent Foundry Vault（创意写卡 / ST 指南库）

`agent-foundry-vault/` 是上游 [agent-foundry-vault](https://github.com/LiarMTTT/agent-foundry-vault) 的本地部署：源文件优先的 SillyTavern 创意流程与知识工坊. 制卡方法论见 skill `tavern-card-builder`；工程化条目与 forge 在 `cards/`；深参见 `st-dev-guide-db`；UI 在 `src/`. 详见 `agent-foundry-vault/本地集成-tavern_helper.md` 与 [`.cursor/skills/CARD-WORKFLOW-STACK.md`](../skills/CARD-WORKFLOW-STACK.md).

### 样式库（UI 片段参考）

`样式库/` 存放可复用的 Vue/HTML/CSS 样式片段（原 `util/found1`、`src/案例`）, 供复制改编, 无 `index.ts` 入口、不参与打包. 详见 `样式库/README.md`.

### 前端项目改造

若要将前端项目改造成带开局流程、MVU 变量、统一请求处理的游戏界面, 参考 `.cursor/rules/前端项目改造指南.mdc`（示例实现见 `备份工作区/ADven`、`备份工作区/Goth`）.
